      ...
      case n =>
        ...
        new AsyncExecutorImpl(managedArrayBlockingQueue.asInstanceOf[BlockingQueue[Runnable]]) {
          override def prioritizedRunnable(priority: => Priority,
                                           run: PrioritizedRunnable.SetConnectionReleased => Unit
                                          ): PrioritizedRunnable = {
            ...
            new PrioritizedRunnable {
              ...
              private def actuallyRun() =
                _run(new PrioritizedRunnable.SetConnectionReleased(() => connectionReleased = true))

              override def run() =
                if (priority() == WithConnection)
                  actuallyRun()
                else {
                  if (managedArrayBlockingQueue.attemptPrepare(this)) {
                    try actuallyRun()
                    finally
